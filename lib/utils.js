...
/**
 * Normalizes and joins the canonical base URL with a relative filename.
 * Ensures a deterministic canonical URL per generated page.
 * @param   {string} filename  Relative output filename (e.g. `index.html`).
 * @returns {string|null}      Absolute canonical URL or `null` if disabled.
 */
function buildCanonicalUrl(filename) {
    if (!view || !view.canonicalBase) return null;

    const rawBase = view.canonicalBase;
    const keepTrailingSlash = Boolean(view.canonicalTrailingSlash);
    const base = rawBase === '/'
        ? '/'
        : rawBase.replace(/\/+$/, '');
    const cleanRelative = (filename || '').replace(/^[./]+/, '');

    if (!cleanRelative || cleanRelative === 'index.html') {
        if (base === '/') return '/';
        return keepTrailingSlash ? `${base}/` : base;
    }

    const normalizedRelative = cleanRelative.replace(/^\/+/, '');
...
/**
 * Updates the current canonical URL on the template view.
 * Clears the value when no canonical base is configured.
 * @param   {string} filename  Relative output filename.
 * @returns {string|null}      The canonical URL that was set, or `null`.
 */
export function setCanonicalUrl(filename) {
    if (!view) return null;

    const canonical = buildCanonicalUrl(filename);
    view.canonicalUrl = canonical || null;

    return canonical;
}

/**
 * Returns a new array with duplicates removed while preserving order.
 * @param   {Array<string>} items  Input array.
...
/**
 * Converts an arbitrary value (HTML or plain text) into a normalized, single-line string.
 * Strips HTML tags, collapses whitespace and trims the result.
 * @param   {string} value  Raw value (HTML or text) to sanitize.
 * @returns {string}        Normalized single-line string or empty string.
 */
function sanitizeDescriptionCandidate(value) {
    if (!value && value !== 0) return '';

    const stringValue = typeof value === 'string' ? value : String(value);
    const withDash = stringValue.replace(/<\/h[1-6]>/i, ' - ');
    return striptags(withDash)
        .replace(/\s+/g, ' ')
        .replace(/\s-\s/, ' - ')
        .trim();
}
...
/**
 * Formats an array into a concise natural-language list.
...
/**
 * Extracts a plain-text summary from a doclet.
 * Prefers class descriptions, then general descriptions, then summaries.
 * @param   {Object} doclet  Doclet to analyze.
 * @returns {string}         Plain text summary or empty string.
 */
function extractDocletSummary(doclet = {}) {
    const summary = doclet.classdesc || doclet.description || doclet.summary;
    return sanitizeDescriptionCandidate(summary);
}

/**
 * Builds a meta description for a source file using related doclets.
 * @param   {Object} fileInfo        Metadata about the source file.
 * @param   {Array<Object>} doclets  Doclets originating from this file.
 * @returns {string}                 Synthesized description string.
 */
function buildSourceDescription(fileInfo, doclets = []) {
    const filename = fileInfo?.shortened || fileInfo?.resolved || 'source';
    const viewTitle = view?.title && typeof view.title === 'string' ? view.title.trim() : '';
    const baseLabel = viewTitle ? `${viewTitle} source file ${filename}` : `Source file ${filename}`;

    if (!doclets.length) return toSeoDescription(`${baseLabel} with inline documentation.`);

    const subjects = unique(
        doclets
            .map(doclet => helper.shortenModuleLinkName(doclet.longname || doclet.name || ''))
            .map(name => name.trim())
            .filter(Boolean)
    );

    const summaryDoclet = doclets.find(doclet => extractDocletSummary(doclet));
    const summary = summaryDoclet ? extractDocletSummary(summaryDoclet) : '';

    let description = baseLabel;
    if (subjects.length) description += ` – contains ${formatList(subjects)}`;
    description += summary ? `. ${summary}` : '.';

    return toSeoDescription(description.trim());
}

/**
 * Builds a meta description for a documentation page based on its primary doclet.
 * @param   {Object} doc  Doclet representing the page content.
 * @returns {string}      SEO-friendly description.
 */
function buildDocDescription(doc) {
    const fallback = view?.description || 'This is the autogenerated JSDoc documentation.';
    const fallbackSeo = toSeoDescription(fallback);

    if (!doc) return fallbackSeo;

    if (doc.kind === 'mainpage' && view?.customDescription) {
        return toSeoDescription(view.customDescription);
    }

    if (doc.kind === 'source' && doc.description) {
        const sourceDesc = sanitizeDescriptionCandidate(doc.description);
        if (sourceDesc) return toSeoDescription(sourceDesc);
    }

    const aggregatedPrimary = sanitizeDescriptionCandidate([
        doc.classdesc,
        doc.description,
        doc.summary
    ].filter(Boolean).join(' '));
    if (aggregatedPrimary) return toSeoDescription(aggregatedPrimary);

    if (doc.kind === 'mainpage' && doc.readme) {
        const readmeText = sanitizeDescriptionCandidate(doc.readme);
        if (readmeText) return toSeoDescription(readmeText);
    }
...
/**
 * Builds a meta description for a tutorial page.
 * @param   {string} title  Rendered tutorial title shown on the page.
 * @param   {string} html   Tutorial body HTML used to derive the summary.
 * @returns {string}        SEO-friendly description.
 */
export function buildTutorialDescription(title, html) {
    const content = sanitizeDescriptionCandidate(html);
    if (content) return toSeoDescription(content.replace(/^[^.-]+ -\s*/, '').trim() || content);

    let fallback = sanitizeDescriptionCandidate(title);
    if (!fallback) fallback = view?.description || 'This is the autogenerated JSDoc documentation.';

    return toSeoDescription(fallback);
}
 ...
 * @returns {string}                  SEO-friendly description.
 */
export function toSeoDescription(text, maxLength = 155) {
    let desc = typeof text === 'string' ? text : String(text ?? '');
...
export function generate(title, docs, filename, resolveLinks, outdir) {
    ...
    const primaryDoc = docs.find(doc => doc.kind === 'mainpage') || docs[0];
    const isHomepage = primaryDoc?.kind === 'mainpage';
    let pageTitle = title;

    view.showPageTitle = !(isHomepage && view.showTitleOnHomepage === false);
    view.desc = buildDocDescription(primaryDoc);
    view.needsAside = needsAside;

    if (isHomepage && typeof primaryDoc?.readme === 'string') {
        const headingMatch = primaryDoc.readme.match(/<(h[1-3])[^>]*>(.*?)<\/\1>/i);
        if (headingMatch && headingMatch[2]) {
            const homepageTitle = sanitizeDescriptionCandidate(headingMatch[2]);
            view.generatedHomepageTitle = homepageTitle;
            view.pageTitleOverride = homepageTitle;
            pageTitle = homepageTitle;
        } else {
            view.generatedHomepageTitle = null;
    ...
    const docData = {
        ...
        title: pageTitle,
        docs,
        aside: needsAside ? view.partial('sidebar.tmpl', { env, title: pageTitle, docs }) : ''
    };
 ...
 * @param {Function} data        Taffy data store with doclets.
 */
export function generateSourceFiles(sourceFiles, encoding, outdir, data) {
    ...
    Object.keys(sourceFiles).forEach(file => {
        ...
        if (typeof data === 'function') {
            relatedDoclets = data().get().filter(doclet => doclet.meta && doclet.meta.shortpath === sourceFiles[file].shortened);
        }

        const description = buildSourceDescription(sourceFiles[file], relatedDoclets);
